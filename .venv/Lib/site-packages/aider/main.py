...
def get_git_root(workspace_path: Optional[str] = None):
    try:
...
def check_gitignore(git_root, io, ask=True, coder=None):
    if not git_root:
        return
...
def main(args=None,
    ...
    git_root = get_git_root(workspace_path=workspace_path)
    if workspace_path:
